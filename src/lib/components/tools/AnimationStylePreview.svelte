<script>
  ...
  function getEstimate() {
    ...
    const estimate = Math.round(
      basePrice * styleMultipliers[selectedStyle] * durationMultipliers[selectedDuration]
    );
    const timeline =
      selectedDuration === 'slow'
        ? '3-4 days'
  ...
  $: estimate =
    selectedStyle && selectedDuration ? getEstimate() : { price: 0, timeline: '1-2 days' };
</script>

<!-- Animation Style Preview Tool -->
<div
  class="bg-gradient-to-br from-lxk-mint/10 to-lxk-sage/10 rounded-3xl p-8 border border-lxk-mint/20"
>
  <div class="text-center mb-8">
    <div
      class="w-16 h-16 bg-gradient-to-br from-lxk-mint to-lxk-sage rounded-2xl flex items-center justify-center mx-auto mb-4"
    >
  ...
  <div class="grid md:grid-cols-2 gap-8">
    ...
    <div class="space-y-6">
        ...
        <div class="space-y-3">
          {#each Object.entries(styles) as [key, style] (key)}
            <button
              ...
              class="w-full text-left p-4 rounded-2xl border-2 transition-all duration-300 hover:shadow-lg {selectedStyle ===
              key
      ...
      <div class="bg-white rounded-2xl p-6 border-2 border-lxk-mint/20">
        ...
        <div class="text-sm text-gray-600">
          Based on: {styles[selectedStyle].name}, {durations[selectedDuration].name.toLowerCase()} timing,
          {colorSchemes[selectedColor].name.toLowerCase()}
        </div>
      </div>
    </div>
    ...
    <!-- Preview Area -->
    <div
      class="bg-white rounded-3xl p-8 border-2 border-gray-100 relative overflow-hidden min-h-[400px]"
    >
      ...
      <div class="relative h-64 flex items-center justify-center">
        ...
        {#key animationKey}
          <div class="relative">
            <!-- Main Shape -->
            <div
              class="w-20 h-20 rounded-2xl shadow-lg {styles[selectedStyle].class} {isPlaying
                ? 'animate-running'
                : ''}"
              style="
            ...
            <!-- Secondary Elements -->
            <div
              class="absolute -top-2 -right-2 w-6 h-6 rounded-full {styles[selectedStyle]
                .class} {isPlaying ? 'animate-running' : ''}"
              style="
            ...
            <div
              class="absolute -bottom-2 -left-2 w-4 h-4 rounded-full {styles[selectedStyle]
                .class} {isPlaying ? 'animate-running' : ''}"
              style="
        ...
        <div class="absolute inset-0 opacity-10">
          <div class="grid grid-cols-8 gap-2 h-full">
            {#each Array(32) as _, i}
              <div
                class="bg-gray-300 rounded-full transform"
