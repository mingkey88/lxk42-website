  ...
  const styles = [
    {
      ...
      description: 'Clean, contemporary design with local flair',
      basePrice: 3500,
    },
    {
      id: 'corporate',
      name: 'Corporate Professional',
      description: 'Trustworthy and established business presence',
      basePrice: 4200,
    },
    {
      id: 'creative',
      name: 'Creative & Artistic',
      description: 'Bold, unique design for creative industries',
      basePrice: 4800,
    },
    {
      id: 'minimalist',
      name: 'Minimalist Zen',
      description: 'Simple, focused, premium aesthetic',
      basePrice: 3800,
    },
  ];
  ...
  const scopes = [
    {
      id: 'landing',
      name: 'Landing Page',
      description: 'Single conversion-focused page',
      multiplier: 0.3,
      weeks: '2-3',
    },
    {
      ...
      description: '5-8 pages with CMS',
      multiplier: 1,
      weeks: '4-6',
    },
    {
    ...
    },
    {
      ...
      description: 'Custom functionality and user accounts',
      multiplier: 2.5,
      weeks: '8-12',
    },
  ];
...
<div
  class="bg-gradient-to-br from-lxk-cream/30 to-lxk-mint/20 rounded-3xl p-8 border border-lxk-sage/10"
>
  <div class="text-center mb-8">
    <div
      class="w-16 h-16 bg-gradient-to-br from-lxk-sage to-lxk-soft-teal rounded-2xl flex items-center justify-center mx-auto mb-4"
    >
  ...
  <div class="space-y-8">
      ...
      <div class="grid md:grid-cols-2 gap-4">
        {#each styles as style (style.id)}
          <button
            ...
            class="text-left p-4 rounded-2xl border-2 transition-all duration-300 hover:shadow-lg {selectedStyle ===
            style.id
          ...
          >
            ...
            <div class="text-sm font-medium text-lxk-peach">
              From SGD ${style.basePrice.toLocaleString()}
            </div>
          </button>
        {/each}
      </div>
      ...
      <div class="grid md:grid-cols-2 gap-4">
        {#each scopes as scope (scope.id)}
          <button
            ...
            class="text-left p-4 rounded-2xl border-2 transition-all duration-300 hover:shadow-lg {selectedScope ===
            scope.id
    ...
    <!-- Industry Selection -->
    <div>
      <label for="industry-select" class="block text-lg font-semibold text-lxk-warm-gray mb-4"
        >Your Industry</label
      >
      <select
        id="industry-select"
      ...
      <div class="grid md:grid-cols-2 gap-3">
        {#each features as feature (feature.id)}
          <button
            ...
            class="text-left p-3 rounded-xl border transition-all duration-200 hover:shadow-md {selectedFeatures.includes(
              feature.id
            )
      ...
      <div class="bg-white rounded-3xl p-6 border-2 border-lxk-sage/20 shadow-lg">
        <div class="text-center mb-6">
          ...
          <div class="text-4xl font-bold text-lxk-sage mb-2">
            SGD ${estimatedCost.toLocaleString()}
          </div>
        ...
        <div class="space-y-3 mb-6">
          <div class="flex justify-between py-2 border-b border-gray-100">
            ...
            <span class="font-semibold text-lxk-warm-gray">
              {styles.find(s => s.id === selectedStyle)?.name} + {scopes.find(
          ...
          {#if selectedFeatures.length > 0}
            ...
            {#each selectedFeatures as featureId (featureId)}
              <div class="flex justify-between py-1 pl-4">
                ...
                <span class="text-lxk-coral"
                  >+SGD ${features.find(f => f.id === featureId)?.price}</span
                >
              </div>
            {/each}
          {/if}
