  ...
  <div class="container-custom">
    ...
    {#if clients.length > 0}
      ...
      <div class="relative overflow-hidden">
        ...
        <div
          class="absolute left-0 top-0 w-20 h-full bg-gradient-to-r from-white via-white/80 to-transparent z-10"
        ></div>
        <div
          class="absolute right-0 top-0 w-20 h-full bg-gradient-to-l from-white via-white/80 to-transparent z-10"
        ></div>
          ...
          {#each duplicatedClients as client, index (client.id + '-' + index)}
            <div class="flex-shrink-0 group relative">
              {#if client.logo}
                <!-- Client Logo -->
                <div
                  class="w-32 md:w-40 h-16 flex items-center justify-center p-4 grayscale hover:grayscale-0 transition-all duration-300 opacity-60 hover:opacity-100"
                >
                  <img
                    src={client.logo}
              ...
              {:else}
                <!-- Text Logo Fallback -->
                <div
                  class="w-32 md:w-40 h-16 flex items-center justify-center p-4 bg-gradient-to-br from-lxk-sage/10 to-lxk-soft-teal/10 rounded-lg opacity-60 hover:opacity-100 transition-all duration-300"
                >
              ...
              {#if showTooltips && client.description}
                <div
                  class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-lxk-warm-gray text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap pointer-events-none z-20"
                >
                  {client.description}
                  <div
