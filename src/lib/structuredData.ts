...
export const reviewSchema = (reviews: Array<any>) => {
  return {
    '@context': 'https://schema.org',
    '@type': 'Organization',
    name: 'Light & Kaki Studio',
...
// Sanitize JSON-LD schema to prevent XSS
const sanitizeSchema = (obj: any): any => {
  ...
  for (const [key, value] of Object.entries(obj)) {
    // Only allow safe property names (alphanumeric, @, -, _)
    if (/^[@\w-]+$/.test(key)) {
...
// Helper function to inject structured data safely
export const injectStructuredData = (
  ...
  if (!schema || typeof schema !== 'object') {
    console.warn('Invalid schema provided to injectStructuredData');
    return;
  }
  ...
  try {
    // Sanitize schema before injection
    const sanitizedSchema = sanitizeSchema(schema);

    // Validate JSON serialization
    const jsonString = JSON.stringify(sanitizedSchema);

    // Additional safety check - ensure it's valid JSON
...
export const updatePageMeta = (title: string, description: string) => {
  ...
  if (ogTitle) (ogTitle as HTMLMetaElement).content = title;

  const ogDescription = document.querySelector('meta[property="og:description"]');
  if (ogDescription) (ogDescription as HTMLMetaElement).content = description;

  // Update Twitter tags
  const twitterTitle = document.querySelector('meta[property="twitter:title"]');
  if (twitterTitle) (twitterTitle as HTMLMetaElement).content = title;

  const twitterDescription = document.querySelector('meta[property="twitter:description"]');
  if (twitterDescription) (twitterDescription as HTMLMetaElement).content = description;
};
